<template>
	<div class="insurance">
		<div class="insurance__box">
			...
			<div class="d-md-flex align-items-center mb-4 insurance__card">
				...
				<div>Du möchtest {{ resultsPage.savingMonth }} € im Monat sparen. Diese solltest du vollständig in einen ETF-Sparplan investieren. Als ETF eignet sich z.B. der MSCI ACWI.</div>
			</div>
		</div>
		...
		<div class="insurance__table mb-5">
			<div class="insurance__table-row bold-line d-flex">
				...
				<p class="py-2 mb-0 font-weight-bold" data-income-res>{{ resultsPage.yourActive }} €</p>
			</div>
			...
			<div class="insurance__table-row small-line d-flex">
				...
				<p class="py-2 mb-0" data-saving>-{{ resultsPage.savingMonth }} €</p>
			</div>
			<div class="insurance__table-row d-flex">
				<p class="bold py-2 mb-0 flex-grow-1">Konsum</p>
				<p class="py-2 mb-0" data-consumption-res>-{{ resultsPage.monthlyExpenses }} €</p>
			</div>
			<div class="insurance__table-row d-flex">
				<p class="bold py-2 mb-0 flex-grow-1 font-weight-bold">Einkommen</p>
				<p class="py-2 mb-0 font-weight-bold" :class="{'success': resultsPage.calculatedInvest() > 0, 'danger': resultsPage.calculatedInvest() <= 0}">{{ resultsPage.calculatedInvest() }} €</p>
			</div>
		</div>

		<div v-if="resultsPage.calculatedInvest() > 0">
			<p class="font-weight-bold mb-4 pb-3">Vermögensentwicklung</p>
			<p class="mb-4 pb-3">Du investierst monatlich {{ resultsPage.savingMonth }} € in einen ETF-Sparplan. Der Wert wird natürlich schwanken, langfristig ist aber von einer durchschnittlichen Steigung von 5 % auszugehen. So wird sich dein Vermögen entwickeln:</p>
	...
	<div v-else>
			<p class="mb-4">Leider fehlen dir {{ Math.abs(resultsPage.calculatedInvest()) }} €, um deine Ziele zu erreichen. Es gibt einige Änderungen, die du vornehmen kannst, um die Rechnung auszugleichen, z.B.: </p>
			<ul class="pl-3">
...
const diagramCalculetedOnYear = () => {
	let currentWealth = 0;
	let increaseOfYourMonth = resultsPage.savingMonth;
	for (let i = 1; i <= 10; i++) {
